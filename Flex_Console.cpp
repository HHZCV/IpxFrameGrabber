...
static void WINAPI GrabbingThread(LPVOID lpUserData,VCECLB_FrameInfoEx* pFrameInfo)
{
	...
	memcpy(ElemQueue.Buffer,pFrameInfo->lpRawBuffer,pFrameInfo->bufferSize);//HZ:copy raw image data to ElemQueue
	GetLocalTime(&ElemQueue.time);

	QUE.push(ElemQueue);//HZ:intert ElemQueue to QUE
	
	////////////////////////////
	// Set Event ImageReady
	////////////////////////////
	SetEvent(hEvent[1]); //HZ: this event saves image, go to SaveThread
}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	/////////////////////////////////////////////
	// Initialization default value , HZ: wcscpy_s is a copy string function
	/////////////////////////////////////////////
	...
	while(i<argc)
	{
		...
		if(wcscmp(argv[i],_T("-p"))==0)
		{
			...
			else
			{
				...
				Port = (char)_wcstoi64(ChPort,NULL,0)-1; //HZ: enter 1,2 for ports but subtract 1, so real ports are 0 and 1
				if(Port != 0 && Port !=1)
				{
		...
		if(wcscmp(argv[i],_T("-h"))==0)
		{
			...
			printf("	-c [FileName]		- Camera config file name\n");
			printf("	-p [1|2]		- Port\n");
...
//////////////////////////////////////////////////////////////
	...
	{
		printf("HZ:cannot execute, cnanot load operating parameters into card");
	}
